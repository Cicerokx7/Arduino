...
void loop() {
 ...
 // Serial1.print("running");
   ...
   //   leftCount = 1;
   // }
    if(angle > zeroAngle-4-error+frontAngle && angle < zeroAngle-4+error+frontAngle){
      frontCheck = distance;
      frontCount = 1;
    }
    if(angle > zeroAngle+4-error+frontAngle && angle < zeroAngle+4+error+frontAngle){
...
if(zeroCount == 1 && mode != 1 && mode != 2 && mode <= 9){
  zeroFirstDistance += zeroDistance;
  mode ++;
}
if(mode == 9){
  zeroFirstDistance = zeroFirstDistance/7;
  mode = 10;
}
if(zeroCount == 1 && mode == 10){
  //1067
  if(zeroWidth < zeroFirstDistance-1067){
    mode = 11;
  }
  zeroCount = 0;
}
...
String Q;
 ...
 Serial.println(mode);
      if(Q=="on"){
        ...
        if(mode == 10){
          if(zeroAngle >= 0 && zeroAngle < 180){
            Serial.println("left");
            Serial3.println("left");
          }
        
          if(zeroAngle > 180 && zeroAngle <= 360){
        ...
        if(mode == 11){
          Serial3.println("stap");
          if(zeroAngle <= rightAngle){
            Serial3.println("rotate");
          }
          else if(zeroAngle > rightAngle && zeroAngle < rightAngle+90){
            Serial3.println("stap");
              rightAngle += 90;
            if(rightAngle == 180){
              straitAngle = straitAngle-90;
            }
            else{
              straitAngle = 360+straitAngle-90;
            }
            mode = 1;
          }
          else{
